...
func generateReadme(p *protogen.Plugin, file *protogen.File, version string) error {
	...
	g.P()
	g.P("⚠️ **VERSION SUPPORT NOTICE**: This action will continue to be supported for Liquibase 4.x. Starting with Liquibase 5.x, it will no longer be supported. ")
	g.P()
	g.P("**Migration to [`liquibase/setup-liquibase`](https://github.com/liquibase/setup-liquibase)**: Available for Liquibase versions 4.32.0 and above. If you're using an older version, upgrade your Liquibase version first.")
	g.P()
	g.P("## Migration Guide")
	g.P()
	g.P("### Current Approach (Liquibase 4.x)")
	g.P("```yaml")
	s := stringy.New(command)
	kebab := s.KebabCase()
	g.P("- uses: liquibase-github-actions/" + kebab.ToLower() + "@v" + version)
	g.P("  with:")
	g.P("    # your parameters here")
	g.P("```")
	g.P()
	g.P("### Recommended Approach (Liquibase 4.32.0+)")
	g.P("```yaml")
	g.P("- uses: liquibase/setup-liquibase@v1")
	g.P("  with:")
	g.P("    version: '" + version + "'  # Requires 4.32.0 or higher")
	g.P("    edition: 'oss'")
	g.P("- run: liquibase " + strings.Replace(command, "_", "-", -1) + " # add your parameters as CLI flags")
	g.P("```")
	g.P()
	g.P("### Migration Steps")
	g.P("1. **Check your Liquibase version**: Ensure you're using 4.32.0 or higher")
	g.P("2. **If using older version**: Update to 4.32.0+ first using the current micro actions") 
	g.P("3. **Then migrate**: Switch to setup-liquibase action")
	g.P()
	g.P("---")
	g.P()
...
func generateDockerfile(p *protogen.Plugin, file *protogen.File, version string) error {
	...
	g.P("# Code generated by protoc-gen-liquibase. DO NOT EDIT - changes will be overwritten.")
...
func generateBashEntry(p *protogen.Plugin, file *protogen.File, globals []protoreflect.FieldDescriptor) error {
	...
	g.P("# Code generated by protoc-gen-liquibase. DO NOT EDIT - changes will be overwritten.")
	g.P()
	g.P("# Version support notice")
	g.P("echo \"::warning title=VERSION SUPPORT NOTICE::This action will stop being supported starting with Liquibase 5.x. To migrate to setup-liquibase, upgrade to Liquibase 4.32.0+ first. See migration guide at https://github.com/liquibase/setup-liquibase\"")
	g.P()
	message := getRequestMessage(file)
	fields := getFieldsFromMessage(message)

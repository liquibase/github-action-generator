...
func generateReadme(p *protogen.Plugin, file *protogen.File, version string) error {
	...
	g.P()
	g.P("⚠️ **VERSION SUPPORT NOTICE**: This action will continue to be supported for Liquibase 4.x. Starting with Liquibase 5.x, it will no longer be supported. We recommend migrating to [`liquibase/setup-liquibase`](https://github.com/liquibase/setup-liquibase) now.")
	g.P()
	g.P("## Migration Guide")
	g.P()
	...
	g.P("```")
	g.P()
	g.P("### Recommended Approach (All Versions)")
	g.P("```yaml")
	g.P("- uses: liquibase/setup-liquibase@v1")
	g.P("  with:")
	g.P("    version: '" + version + "'  # Use same version as current approach")
	g.P("    edition: 'oss'")
...
func generateBashEntry(p *protogen.Plugin, file *protogen.File, globals []protoreflect.FieldDescriptor) error {
	...
	g.P("echo \"::warning title=VERSION SUPPORT NOTICE::This action will stop being supported starting with Liquibase 5.x. Migrate to https://github.com/liquibase/setup-liquibase\"")
	g.P()
	message := getRequestMessage(file)
	fields := getFieldsFromMessage(message)

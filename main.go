...
func generateReadme(p *protogen.Plugin, file *protogen.File, version string) error {
	...
	g.P()
	g.P("⚠️ **DEPRECATED**: This action will be discontinued with Liquibase 5.0. Please migrate to [`liquibase/setup-liquibase`](https://github.com/liquibase/setup-liquibase) for continued support.")
	g.P()
	g.P("## Migration Guide")
	g.P()
	g.P("### Old Approach (This Action - DEPRECATED)")
	g.P("```yaml")
...
func generateBashEntry(p *protogen.Plugin, file *protogen.File, globals []protoreflect.FieldDescriptor) error {
	...
	g.P("# Code generated by protoc-gen-liquibase. DO NOT EDIT.")
	g.P()
	g.P("# Deprecation warning")
	g.P("echo \"::warning title=DEPRECATION NOTICE::This Liquibase action is deprecated and will be discontinued with Liquibase 5.0. Please migrate to https://github.com/liquibase/setup-liquibase for continued support.\"")
	g.P()
	message := getRequestMessage(file)
	fields := getFieldsFromMessage(message)

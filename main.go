...
func generateReadme(p *protogen.Plugin, file *protogen.File, version string) error {
	...
	g.P()
	g.P("⚠️ **VERSION SUPPORT NOTICE**: This action supports Liquibase versions up to 4.x. For Liquibase 5.0+ features, please migrate to [`liquibase/setup-liquibase`](https://github.com/liquibase/setup-liquibase).")
	g.P()
	g.P("## Migration Guide")
	g.P()
	g.P("### Current Approach (Supports Liquibase 4.x)")
	g.P("```yaml")
	...
	g.P("```")
	g.P()
	g.P("### Recommended for Liquibase 5.0+ Features")
	g.P("```yaml")
	g.P("- uses: liquibase/setup-liquibase@v1")
	g.P("  with:")
	g.P("    version: '5.0.0'  # Supports latest features")
	g.P("    edition: 'oss'")
...
func generateBashEntry(p *protogen.Plugin, file *protogen.File, globals []protoreflect.FieldDescriptor) error {
	...
	g.P("echo \"::warning title=VERSION SUPPORT NOTICE::This Liquibase action supports versions up to 4.x. For Liquibase 5.0+ features, migrate to https://github.com/liquibase/setup-liquibase\"")
	g.P()
	message := getRequestMessage(file)
	fields := getFieldsFromMessage(message)
